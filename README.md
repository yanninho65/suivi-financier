...
**Version documentée : `v 2026.09.11.17.22`** — à comparer à `APP_VERSION` en tête du `<script>` du fichier `index.html` (format `v AAAA.MM.JJ.HH.MM`, affichée en petit sous le titre "Suivi financier" sans le préfixe `v `). Mise à jour uniquement sur demande explicite : si les versions diffèrent, vérifier en priorité §2, §4, §5, §6.1, §6.2, §6.3, §6.4, §6.6, §6.7, §7.2 et §8 (zones les plus souvent modifiées).
...
**Budget prédéfini — vue Amorti/Réel/Les deux** (`Store.settings.budget.compare.predefinedVue`, objet clé→vue, même clé que `compare.predefined` — `predefinedCatKey`/`predefinedVoyageKey`) : chaque montant prédéfini peut être associé à l'une des deux vues du Budget plutôt qu'affiché identique dans les deux. Réglable via 3 boutons sous chaque montant, dans la fiche "Budget prédéfini" (`openPredefinedBudgetSheet`) :
- **Les deux** (`"both"`, valeur par défaut) : montant affiché identique quel que soit le mode affiché.
...
- **Réel** (`"reel"`) : montant affiché tel quel en vue "Ajouts" ; en vue "Amort. + ajouts", cette part est **rajoutée** (montant + amortissement).
...
*Exemple* : un montant prédéfini de 1000 € de dépense scopé "Réel", avec 700 € d'amortissement calculés sur cette même ligne, affiche **1000 €** en vue "Ajouts" et **1700 €** en vue "Amort. + ajouts". Scopé "Amorti", il afficherait **1000 €** en "Amort. + ajouts" et **300 €** en "Ajouts". Scopé "Les deux", 1000 € dans les deux vues. Persisté en Excel (colonne "Vue" de `Budget_Predefini`, §7/§8) et en JSON/Gist (`comparePredefinedVue`, §7.2) — fusionné clé par clé comme `compare.predefined` en mode "Ajouter" (jamais écrasé si déjà renseigné localement).
...
**Santé (§6.6) a son propre flux d'import Excel/CSV, entièrement séparé** — colonnes, dédoublonnage et écran dédiés, sans réconciliation casse/accents ; les soins ne transitent par ce §7 que de façon transparente lors d'une restauration `.json` complète (§6.6, §8).
...
**Feuille `Budget_Predefini`** : colonne `Vue (Amorti/Réel/Les deux)` (§6.3.5) — texte simple, absente ou toute valeur non reconnue relue comme "Les deux" à l'import.
...
- Réconciliation d'import (§7.1) : ne couvre que Catégorie/Personne/Compte/Compte cible/Groupe — pas Remarque, pas Type. Ne s'exécute que pour un import Excel/CSV avec des transactions déjà présentes ; une restauration `.json` ne passe jamais par cette étape.
- Onglet Santé (§6.6) : la fusion à l'import ignore silencieusement un soin dont l'id existe déjà — elle ne met jamais à jour un soin existant (ex. un `termine` basculé localement puis un import "Ajouter" du même fichier source ne l'écrase pas). Pas de réconciliation casse/accents comme en §7.1.
