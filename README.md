...
- Si une date manque, ou si `dateTo < dateFrom` (jours ≤ 0) : aucun calcul — message "Renseignez les deux dates…" à la place. Le calcul est fait en direct dans l'écran d'édition (`createAliasManager`, §2.4) ; l'ancien helper `AGG.getGroupDayStats`, jamais appelé, a été supprimé lors de l'audit (§15).
...
**Groupe / Personne** : liste imbriquée (`nestedFilterHtml`/`wireNestedFilter`, simples enveloppes de `U.hierarchyCheckListHtml`/`U.wireHierarchyCheckList` — rendu et comportement **communs** avec le filtre Groupe de Santé §6.6 et celui de Revenus §6.7.5), groupes de groupe/personnes repliés par défaut, cochables au niveau du cluster ou d'un élément précis. Un groupe sans groupe de groupe (liste "à plat") réaffiche correctement sa case cochée à la réouverture du filtre (corrigé lors de l'audit, §15). **Personne** uniquement : les personnes non rattachées apparaissent sous un en-tête dédié **"Sans type"** (`NESTED_DIM_CONFIG.personnes.noGroupLabel`), cliquable pour tout sélectionner en bloc (bascule directe de chaque membre).
...
**Exceptions** : l'onglet Budget affiche toujours 0 décimale, indépendamment de ce réglage. L'onglet Évolution aussi, ainsi que Récurrent et Revenus, via le formateur partagé `U.fmtWholeSigned` (alias locaux `fmtVal`/`recFmtVal`/`revFmtVal`) et la classe de couleur `U.signClass` — indépendants de `Store.settings.decimals`.
...
- Badges **Réel / Amorti / Ajout** en haut (totaux) — cliquables, servent de filtre sur la liste en dessous. Rendu commun aux 4 feuilles de détail du Budget (catégorie, voyage, Revenus/Dépenses/Voyages/Total, mois de la vue mensuelle) : `openBudgetKindSheet(o)` + `BG_KIND_STYLE` (§12) — chaque feuille ne fait plus que collecter ses lignes.
...
**Groupe** (`openRevFilterSheet`) : groupé par groupe de groupe via `AGG.groupeHierarchyChrono`, ordre chronologique (§2.1 bis), sélectionnable au niveau du groupe de groupe (en-tête) ou d'un groupe précis, avec recherche. Même composant que le filtre Groupe des Transactions (`U.hierarchyCheckListHtml`/`U.wireHierarchyCheckList`, §4) ; la sélection n'est enregistrée qu'au clic sur "Voir les résultats". Stocké dans `groupFilter` (tableau de noms, mélange groupe de base et groupe de groupe). Ne s'applique qu'aux lignes récurrentes : `AGG.buildRecurringIndex(periodFrom, periodTo, groupFilter)` teste `t.groupeBase` **et** `getGroupeDisplay(t)` — même sémantique à deux niveaux que le filtre Groupe de Transactions (§2.3).
...
|---|---|---|
| Utils | `window.__SF_UTILS` | formatage, dates, icônes SVG, `baseGroupe()`, `baseRemarque()` (nom de transaction sans suffixe de date, §6.5), `computeTransform()`, `enableDragScroll(el)` (glisser-déposer horizontal, §11), `decimalsToggleBtnHtml()`/`toggleMoneyDecimals()` (§5), `hideZeroAccountsBtnHtml()`/`toggleHideZeroAccounts()`/`isZeroBalance(bal)` (§6.1) ; **helpers partagés** (§15) : `capitalize`/`fmtMonthYearCap`, `fmtWholeSigned`/`signClass` (§5), `isoToDateObj`/`setDateColumnFormat` (§8), `hierarchyCheckListHtml`/`wireHierarchyCheckList` (§4), `xlsxAvailable()`/`XLSX_MISSING_MSG` (§14) |
...
| Vue Budget | idem, dans `window.__SF_VIEWS.renderBudget` | 2 modes (Amort.+ajouts / Ajouts), vue détail 3 colonnes, `computeGroupBudget()`, `groupSelectionMatches()`/`adjustmentGroupMatches()` (correspondance groupe/ajustement avec repli hiérarchique, §2.1/§6.3.6), `salaryMonthBreakdown()`, `groupPickerHierarchy()` (picker groupe principal/additionnel, alpha, §6.3.2), `adjustmentGroupHierarchy()` (picker groupe d'un ajustement, chronologique + icône type, §6.3.6), `openBudgetKindSheet(o)`/`BG_KIND_STYLE` (feuille de détail Réel/Amorti/Ajout commune, §6.3.3), `voyageShareInMonths(t, monthSet)` (part d'une transaction voyage sur une période, sur `voyageTxnMonths`) |
...
- Ressources externes (Google Fonts, SheetJS/cdnjs) : cache séparé non versionné (`sf-runtime`), stratégie "stale-while-revalidate" pour un usage hors ligne après un premier chargement en ligne. Seules les réponses exploitables (`res.ok` ou opaques cross-origin) sont mises en cache — une erreur 4xx/5xx du CDN ne remplace jamais une copie valide.
- SheetJS est chargé avec `defer` : il ne bloque plus le premier affichage (≈ 900 Ko de script), et reste disponible avant `DOMContentLoaded`, donc avant `init()`. Il n'est de toute façon utilisé qu'à l'import/export (`U.xlsxAvailable()` garde les 4 points d'entrée : `DATA.parseFile`, `exportExcel`, export et import Santé).
...
## 15. Audit de code (2026-09-24, `v 2026.09.24.12.09`)
Passe de nettoyage sans changement fonctionnel visible (hors correctifs listés), validée par un parcours Playwright automatique de l'interface (5 configurations × 7 onglets, clic sur chaque élément interactif puis sur chaque élément des feuilles ouvertes, empreinte du DOM comparée avant/après) et par des tests ciblés comparés à l'identique avant/après (persistance, sync Gist simulée — création, à jour, distant/local modifié, conflit, erreur réseau —, filtres Groupe/Personne/Santé/Revenus, 4 feuilles de détail du Budget dans 4 configurations, import Excel, réglages exportés).

**Code mort supprimé** : `AGG.computeBreakdown`, `AGG.computeTimeSeries` (+ `iterateBuckets`), `AGG.groupeHierarchy` (+ `getSuperGroupeForBase`), `AGG.distinctValuesPersonneDisplay`, `AGG.getGroupDayStats` ; `render()` et l'état `view` jamais lus de `createAliasManager` ; variables locales et imports de module inutilisés ; règle CSS `.seg-xs`.

**Redondances factorisées** (une seule implémentation, réutilisée) :
